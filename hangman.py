...
def play(word):
    ...
    guessed = False  # сигнальнвя метка
    guessed_letters=[]  # список названных букв
    ...
    print(f'Слово состоит из {len(word)} букв', word_complection, sep=' ')
    ...
    while True:
        ...
        if not word_input.isalpha():
            print('Ты слепой баклан, что ты вводишь!!! Попробуй ещё раз!!!')
            continue
        ...
        if len(word_input) > 1:
            if word_input == word:
                print('Я поздравляю тебя, мудак. Ты угадал слово. Ты победил.')
                break
            else:
                guessed_words.append(word_input)
                tries = tries - 1
                print(f'Ты ошибся, мешок с говном. У тебя осталась {tries} попыток.')
                print(display_hangman(tries))
                print_word(word, guessed_letters)
                if tries == 0:
                    print(f"Мудак! Ты проиграл. Загадано было слово: {word}")
                    break
                continue
